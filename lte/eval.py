## python lte/eval.py --generation_path save/genration/01M_20D_14H_10m --output_path save/output/01M_20D_11H_31m
...
def print_dict_in_table_format(data,benchmark_name_max_len,task_name_max_len,metric_max_len):
    column_widths = [benchmark_name_max_len+2,task_name_max_len+10, metric_max_len+5, 10]  # 定义每列的宽度
    header = ["BenchMark","Tasks", "Metric", "Value"]
    # 打印表头
    logger.info("|{}|{}|{}|".format(
        header[0].center(column_widths[0], ' '),
        header[1].center(column_widths[1], ' '),
        header[2].center(column_widths[2], ' '),
        header[2].center(column_widths[3], ' ')
    ))
    logger.info("|{}|{}|{}|".format(
        "-" * column_widths[0],
        "-" * column_widths[1],
