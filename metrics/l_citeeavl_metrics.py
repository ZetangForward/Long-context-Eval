...
class L_cite_eavl_Counting_Stars:
    ...
    def __call__(self, passage, ground_truth, results):
        score = {"f1":0,"recall":0,"precision":0,"acc":0}
        gold_ind_lst = []
        gold_ans_lst = []
        total_correct=0
        ...
        if precision + recall == 0:
            ...
            f1 = 2 * precision * recall / (precision + recall)
        score["f1"] =f1;score["precision"]=precision;score["recall"]=recall
        ...
        for idx, ans in enumerate(model_ans['little_penguin']):
            if ans in gold_ans_lst:
                total_correct += 1
        score["acc"]=total_correct/len(gold_ans_lst)
        return score
...
class L_cite_eavl_cite:
    ...
    def __call__(self, passage, ground_truth, results):
        ...
        score["f1"]=citation_f1
        score['precision']=entail_prec / total_citations if total_citations > 0 else 0
...
class L_cite_eavl_counting_stars_cite:
    ...
    def __call__(self, passage, ground_truth, results):
        ...
        score["recall"]=recall
        score['cite_num']=len(model_ans['passage_id'])
        return  score
